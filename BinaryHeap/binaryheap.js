...
BinaryHeap.prototype.insert = function(value) {
  ...
  const verifyAndSwap = index => {
    index = index || this._heap.length - 1;
    ...
    if (this._compare(this._heap[index], this._heap[parentIndex])) {
      ...
      return verifyAndSwap(parentIndex);
    }
    return this._heap;
  };
...
BinaryHeap.prototype.removeRoot = function() {
  ...
  const popedValue = this._heap.pop();
  const compareAndSwap = index => {
    index = index || 0;
    const firstChildrenIndice = index * 2 + 1;
    const secondChildrenIndice = index * 2 + 2;
    if (this._compare(this._heap[firstChildrenIndice], this._heap[index])) {
      [this._heap[firstChildrenIndice], this._heap[index]] = [this._heap[index], this._heap[firstChildrenIndice]];
      return compareAndSwap(firstChildrenIndice);
    } else if (this._compare(this._heap[firstChildrenIndice], this._heap[index])) {
      [this._heap[secondChildrenIndice], this._heap[index]] = [this._heap[index], this._heap[secondChildrenIndice]];
      return compareAndSwap(secondChildrenIndice);
    }
    return popedValue;
  };
